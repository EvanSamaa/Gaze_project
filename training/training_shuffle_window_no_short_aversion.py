...
def train_model_with_vel(model, config, dataset_location, training_set, testing_set, wandb, model_name, start = 1):
    ...
    training_dataset = Runtime_parsing_Aversion_SelfTape111(dataset_location, training_set, long_aversion_only = True)
    validation_dataset = Runtime_parsing_Aversion_SelfTape111(dataset_location, testing_set, long_aversion_only = True)
    ...
    for epoch in range(start, config['epochs'] + 1):
        ...
        training_dataset = Runtime_parsing_Aversion_SelfTape111(dataset_location, training_set, training_dataset, window_length=new_win_size, long_aversion_only = True)
        validation_dataset = Runtime_parsing_Aversion_SelfTape111(dataset_location, testing_set, validation_dataset, long_aversion_only = True)
        train_data = torch.utils.data.DataLoader(training_dataset, config['batch_size'], True)
        valid_data = torch.utils.data.DataLoader(validation_dataset, config['batch_size'], True)
        total_train_loss = 0
        total_valid_loss = 0
...
train_model_with_vel(model, config, dataset_location, training_set, testing_set, run_obj, "reparse_dataset_explicit_first_frame_with_pos_long_aversion_only")
run_obj.finish()
