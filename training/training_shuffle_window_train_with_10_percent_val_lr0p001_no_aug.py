...
def train_model_with_vel(model, config, dataset_location, training_set, testing_set, wandb, model_name, start = 1):
    training_dataset = Runtime_parsing_Aversion_SelfTape111_validation_leak(dataset_location, training_set, normalize_MFCC=False, apply_frequency_mask=False, percent_leaked=0.10)
    validation_dataset = Runtime_parsing_Aversion_SelfTape111(dataset_location, testing_set, normalize_MFCC=False)
    ...
    for epoch in range(start, config['epochs'] + 1):
        ...
        training_dataset = Runtime_parsing_Aversion_SelfTape111_validation_leak(dataset_location, training_set, training_dataset, window_length=new_win_size, normalize_MFCC=False, apply_frequency_mask=False)
        validation_dataset = Runtime_parsing_Aversion_SelfTape111(dataset_location, testing_set, validation_dataset, normalize_MFCC=False)
        train_data = torch.utils.data.DataLoader(training_dataset, config['batch_size'], True)
        valid_data = torch.utils.data.DataLoader(validation_dataset, config['batch_size'], True)
        total_train_loss = 0
        total_valid_loss = 0
...
train_model_with_vel(model, config, dataset_location, training_set, testing_set, run_obj, "reparse_dataset_explicit_first_frame_10percent_leak_lr0p001_no_aug", start=1)
run_obj.finish()
