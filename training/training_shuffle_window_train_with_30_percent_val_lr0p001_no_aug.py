...
def train_model_with_vel(model, config, dataset_location, training_set, testing_set, wandb, model_name, start = 1):
    training_dataset = Runtime_parsing_Aversion_SelfTape111_validation_leak(dataset_location, training_set, normalize_MFCC=False, apply_frequency_mask=False, percent_leaked=0.30)
...
train_model_with_vel(model, config, dataset_location, training_set, testing_set, run_obj, "reparse_dataset_explicit_first_frame_30percent_leak_lr0p001_no_aug", start=1)
run_obj.finish()
