...
def train_model_with_vel(model, config, dataset_location, training_set, testing_set, wandb, model_name, start = 1):
    training_dataset = Runtime_parsing_Aversion_SelfTape111(dataset_location, training_set, normalize_MFCC=True, apply_frequency_mask=True, apply_time_mask=True)
    ...
    for epoch in range(start, config['epochs'] + 1):
        ...
        training_dataset = Runtime_parsing_Aversion_SelfTape111(dataset_location, training_set, training_dataset, window_length=new_win_size, normalize_MFCC=True, apply_frequency_mask=True, apply_time_mask=True)
...
train_model_with_vel(model, config, dataset_location, training_set, testing_set, run_obj, "reparse_dataset_explicit_first_frame_with_freq_mask_and_time_mask", start=1)
run_obj.finish()
