...
def get_velocity(out):
    ...
    return vel
def train_model_with_vel(model, device, config, train_data, valid_data, wandb, model_name, start = 1):
    ...
    for epoch in range(start, config['epochs'] + 1):
        ...
        for _, (X, [Y, Y_vel]) in enumerate(train_data):
            ...
            loss = loss + 0.3 * (loss_fn_vel(pred_vel[:, :, 0], -Y_vel) + loss_fn_vel(pred_vel[:, :, 1], Y_vel))
            loss.backward()
            optimiser.step()
...
if __name__ == "__main__":
    ...
    model = SentenceBaseline_GazePredictionModel(config, device)
    ...
    train_model_with_vel(model, device, config, train_dataloader, valid_dataloader, run_obj, "sentence_and_words_with_vel_2023_Oct_prevent_overfitting_larger_model_Oct10_13_19_no_BN_for_LSTM")
    run_obj.finish()