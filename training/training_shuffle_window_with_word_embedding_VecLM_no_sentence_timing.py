...
# Model
class Explicit_context_GazePredictionModel_mel_only_with_word_vec_no_sentence_timing(nn.Module):
    def __init__(self, config):
        torch.set_default_tensor_type(torch.DoubleTensor)
        # initialize model
        super(Explicit_context_GazePredictionModel_mel_only_with_word_vec_no_sentence_timing, self).__init__()
        ...
        self.lstm = nn.LSTM(2 * (64 + config["input_layer_out"] + config["pos_feature_size"]) * (self.frames_ahead + self.frames_behind + 1), 
    ...
    def forward(self, input_feature, initial_state):
        ...
        x_combined = torch.cat([x1_windowed, x1_word_vector_windowed, pos_features_self, x2_windowed, x2_word_vector_windowed, pos_features_other], dim=2)
...
def train_model_with_vel(model, config, dataset_location, training_set, testing_set, wandb, model_name, start = 1, device_string="cuda:0"):
    ...
    training_dataset = Runtime_parsing_Aversion_SelfTape111_with_word_vec(dataset_location, training_set, long_aversion_only = True)
    validation_dataset = Runtime_parsing_Aversion_SelfTape111_with_word_vec(dataset_location, testing_set, long_aversion_only = True)
    ...
    for epoch in range(start, config['epochs'] + 1):
        ...
        training_dataset = Runtime_parsing_Aversion_SelfTape111_with_word_vec(dataset_location, training_set, training_dataset, window_length=new_win_size, long_aversion_only = True)
        validation_dataset = Runtime_parsing_Aversion_SelfTape111_with_word_vec(dataset_location, testing_set, validation_dataset, long_aversion_only = True)
        train_data = torch.utils.data.DataLoader(training_dataset, config['batch_size'], True)
        valid_data = torch.utils.data.DataLoader(validation_dataset, config['batch_size'], True)
        total_train_loss = 0
        total_valid_loss = 0
...
model = Explicit_context_GazePredictionModel_mel_only_with_word_vec_no_sentence_timing(config)
if config["wandb"]:
    wandb.login()
    run_obj = wandb.init(project="gaze_prediction", config=config, settings=wandb.Settings(start_method="fork"))

train_model_with_vel(model, config, dataset_location, training_set, testing_set, run_obj, "reparse_dataset_explicit_first_frame_with_pos_long_aversion_only_word_embedding_WavLM_no_sentence_timing", device_string=device_string)
run_obj.finish()
