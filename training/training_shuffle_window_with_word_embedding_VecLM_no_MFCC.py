...
# Model
class Explicit_context_GazePredictionModel_mel_only_with_word_vec_no_MFCC(nn.Module):
    def __init__(self, config):
        torch.set_default_tensor_type(torch.DoubleTensor)
        # initialize model
        super(Explicit_context_GazePredictionModel_mel_only_with_word_vec_no_MFCC, self).__init__()
        ...
        self.lstm = nn.LSTM(2 * (64 + 6 + config["pos_feature_size"]) * (self.frames_ahead + self.frames_behind + 1), 
    ...
    def forward(self, input_feature, initial_state):
        ...
        x_combined = torch.cat([x1_word_vector_windowed, pos_features_self, text_feature_self, x2_word_vector_windowed, pos_features_other, text_feature_other], dim=2)
...
model = Explicit_context_GazePredictionModel_mel_only_with_word_vec_no_MFCC(config)
if config["wandb"]:
    wandb.login()
    run_obj = wandb.init(project="gaze_prediction", config=config, settings=wandb.Settings(start_method="fork"))

train_model_with_vel(model, config, dataset_location, training_set, testing_set, run_obj, "reparse_dataset_explicit_first_frame_with_pos_long_aversion_only_word_embedding_WavLM_no_MFCC", device_string=device_string)
run_obj.finish()
