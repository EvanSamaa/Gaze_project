...
def obtain_items_of_interest_time_varying(manual_select, look_through=True):
    ...
    # get speaker ids
    output_json_speaker_id = {}
    for i in range(0, len(out_selected_item_names_list)):
        keyframes = cmds.keyframe(out_selected_item_names_list[i]+".SpeakerId", query=True, timeChange=True)
        speaker_ids = []
        if not keyframes is None:
            for frame in keyframes:
                value = cmds.getAttr(out_selected_item_names_list[i]+".SpeakerId", time=frame)
                speaker_ids.append([float(frame)/fps, value])
        if speaker_ids == []:
            speaker_ids = [[0, cmds.getAttr(out_selected_item_names_list[i]+".SpeakerId", time=0)]]
        output_json_speaker_id[out_selected_item_names_list[i]] = speaker_ids  
        print(speaker_ids)
    ...
    output_json = {
        ...
        "speaker_id": output_json_speaker_id,
        "cam_pos": cam_pos
    }
    global file_path
...
def load_scene(*args):
    ...
    temp_speaker_ids = file_content["speaker_id"]
    for key in temp_object_type.keys():
        ...
        speaker_id = temp_speaker_ids[key]
        ...
        cmds.addAttr(sp, ln='SpeakerId', at="number", k=True)
        try:
            ...
            cmds.setKeyframe(sp, attribute='object_interestingness', t=time, v=value)
        try:
            for pair in speaker_id:
                time = pair[0]
                value = pair[1]
                # add a keyframe with the corresponding time and value
                cmds.setKeyframe(sp, attribute='SpeakerId', t=time, v=value)
        except:
            time = 0
            value = speaker_id
            # add a keyframe with the corresponding time and value
